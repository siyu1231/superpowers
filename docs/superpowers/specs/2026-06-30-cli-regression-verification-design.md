...
```
brainstorming ──→ design.md + regression-cases.json
     │                    （CLI 入口是硬性要求）
     ▼
...
SDD (subagent-driven-development)
  ...
  └─→ 【人工闸门】"是否进入验证阶段？"
      ...
      regression-guard: 当前功能验证
         ├─ 确定性验证: TOGGLE=0 / TOGGLE=1 CLI 执行 → 对比期望
         ├─ 失败 → 子 agent (systematic-debugging) 修复代码 → 重新验证
         │        循环直到通过（单案例最多 3 轮）
         ├─ 不确定性验证（如标记需要）:
         │   子 agent (clean-context-verification) 干净上下文验证
         │   └─ 失败 → 子 agent (debug) 修复 → 重新验证
         └─ 全部通过 → 写入案例库
              ├─ 新增案例: append 到 regression/cases.json
              └─ 同 id 案例但期望变化: update 对应条目
     ▼
finishing-a-development-branch
  │
  └─→ 【全量回归闸门】
       regression-guard: 全量回归
         ├─ 运行案例库中所有历史案例
         └─ 失败 → 子 agent (debug) 修复 → 重新验证（循环直到通过）
```
